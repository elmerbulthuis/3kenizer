			...
			*/
			...
			function addHandler(categories, handler){
				var track = {
					...
					, position: writeBufferPosition + (currentTrack ? currentTrack.match.index + currentTrack.match[0].length : 0)
					, token: null
				};
				tracks.push(track);
			}//addHandler
			...
			function handleTrack(track){
				...
				if(match && match.position == track.position){
					track.match = match;
					currentTrack = track;
			...
			function findMatch(categories, offsetPosition){
				...
				.map(function(category, categoryIndex){
					return findCategoryMatch(category, offsetPosition);
				})
